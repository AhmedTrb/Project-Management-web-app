...
const calculateMPM = (projectId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const reverseAdjList = new Map(); // dependent → prereqs
    ...
    for (const id of topoOrder.slice().reverse()) {
        ...
        if (node.dependents.length === 0) {
            ...
            node.latestFinish = Math.min(...node.dependents.map(d => nodes.get(d).latestStart));
        }
