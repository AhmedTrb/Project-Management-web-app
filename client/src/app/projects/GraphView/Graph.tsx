...
export default function Graph({ id }: Props) {
  ...
  const { initialNodes, initialEdges } = useMemo(() => {
    ...
    tasks.forEach(({ id, degree }) => {
      const predecessors = predecessorsMap.get(id) || [];
...
type TaskNodeProps = {
  ...
  setNodes: React.Dispatch<React.SetStateAction<TaskNode[]>>;
  nodes: TaskNode[];
};
...
const TaskNodeCard = ({ data, setNodes, nodes }: TaskNodeProps) => {
              ...
              <div className="absolute top-5 right-0 bg-white shadow-md rounded-md p-2 w-32 z-40">
                <div 
                  ...
                  onClick={() => {
                    ...
                    setNodes(nodes.filter((node) => node.id !== data.id.toString()));
                  }}
                >
                  Delete
                </div>
